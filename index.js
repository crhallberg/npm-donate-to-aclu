...
function validate(formdata) {
  return new Promise(function(succeed, fail) {
    ...
    if (formdata.amount) {
      formdata.amount =
        Math.floor(
          parseFloat(String(formdata.amount).replace(/[^\d.,]/g, "")) * 100
        ) / 100;
      if (!formdata.amount) {
      ...
      if (formdata.amount < 5) {
        return fail(
          "Sorry, only donations $5 or larger can be processed ($" +
            formdata.amount +
            ")."
        );
      }
    ...
    if (formdata.cc_exp) {
      if (formdata.cc_exp instanceof Date) {
        formdata.cc_exp =
          formdata.cc_exp.getMonth() + "/" + formdata.cc_exp.getYear() % 100;
      }
    ...
    if (!stateIndexes[formdata.state]) {
      return fail(
        "Please provide the full state name for " + formdata.state + "."
      );
    }
    // Invalid country
    if (
      formdata.country_code &&
      typeof formdata.country_code === "string" &&
      formdata.country_code.match(/\D/g)
    ) {
      return fail(
        "Please look up your country's numerical code: https://en.wikipedia.org/wiki/ISO_3166-1_numeric#Officially_assigned_code_elements."
      );
    }
    ...
    if (
      ...
      return fail(
        "Credit card number or security code is not valid (" +
          formdata.cc_number +
          ", " +
          formdata.cc_code +
          ")"
